"""CLI entry point for Nidus Modular RAG — index, ask, interactive modes."""
...
def cmd_index(args):
    ...
    print("=" * 60)
    print("  Nidus Modular RAG — Index Builder")
    print("=" * 60)
    ...
    print(
        f"  → {len(chunks)} chunks created (size={chunker_cfg['chunk_size']}, "
        f"overlap={chunker_cfg['chunk_overlap']})"
    )
...
def _print_result(result):
    ...
    print(f"  路由: {result.route}" + (f"（{result.reason}）" if result.reason else ""))
    print("=" * 60)
    print(f"\n{result.answer}")
...
def cmd_ask(args):
    ...
    result = graph_ask(args.question)
...
def cmd_interactive(args):
    ...
    print("=" * 60)
    print("  Nidus Modular RAG — Interactive Mode")
...
def main():
    parser = argparse.ArgumentParser(
        prog="nidus",
        description="Nidus Modular RAG — LangGraph 动态路由模块化编排",
    )
    ...
    index_parser.add_argument(
        ...
        choices=["fixed"],
        default="fixed",
        help="Chunking strategy (Modular 阶段仅支持 fixed)",
    )
    ...
    interactive_parser = subparsers.add_parser(
        "interactive", help="Interactive Q&A mode"
    )
